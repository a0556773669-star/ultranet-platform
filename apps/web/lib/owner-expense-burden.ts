/**
 * The owner's economic burden (ownerExpenseBurden, driven by `owedBy` - never the partner's
 * share) from *fixed/recurring* branch expenses across both the rentals (ניידים) and
 * computer-rooms (נייחים) modules, including each module's "shared / all branches" pool.
 *
...
import { ownerExpenseBurden, monthsBetween } from "./branch-accounting";
...
export async function loadOwnerFixedExpenseBurden(): Promise<OwnerFixedExpenseBurden> {
  ...
  for (const doc of fixedSnap.docs) {
    ...
    const burden = ownerExpenseBurden(amount, e.owedBy);
    ...
    toDate += burden * activeMonths.length;
    if (activeMonths.includes(month)) thisMonth += burden;
  }
