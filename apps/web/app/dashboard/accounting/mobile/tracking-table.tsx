  ...
  return (
    <div className="overflow-hidden rounded-card border border-card-border bg-white shadow-card">
      ...
      <p className="border-t border-card-border px-4 py-2.5 text-[11px] leading-relaxed text-muted">
        המספר בתא = הרווח התפעולי שלי מהסניף באותו חודש, מחולק במספר המחשבים שהיו בו באותו חודש
        (מחשב שנוסף באמצע נספר רק מהחודש שנוסף בו). ירוק = עומד ביעד של {target} ₪ למחשב לחודש
        (150 ₪ + מע&quot;מ). תא ריק = הסניף עדיין לא היה קיים.
        <br />
        <b>מה לא נספר כאן:</b> הוצאה שאני שילמתי והחוב כולו עליי — כלומר רכש (קניית מחשבים וציוד).
        זו השקעה בסניף, לא עלות תפעול, וחודש שנקנו בו שני מחשבים היה נראה כאן כמו חודש הפסד בסניף
        שתפקד בדיוק כרגיל. הוצאות משותפות והוצאות שהסניף נושא בחלקן — כן נספרות.
      </p>
    </div>
  );
}
