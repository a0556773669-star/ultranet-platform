      ...
      <section className={CARD}>
        <div className="flex flex-wrap items-center justify-between gap-2.5 border-b border-card-border px-4 py-3">
          <div>
            ...
            <p className="mt-0.5 text-[12.5px] text-muted">
              כל שורה נשמרת בנפרד. אחרי שמירה הנתונים במסכי ההנה&quot;ח מתעדכנים מיד. המסך הזה פתוח
              ל{ownerName} בלבד — בעלי הסניפים רואים את תאריך הפתיחה אך לא יכולים לשנות אותו.
            </p>
          </div>
          <Link
        ...
        <div className="border-t border-card-border px-4 py-3 text-[11.5px] leading-relaxed text-muted">
          <b className="text-ink">איך זה עובד:</b> ברגע שהוזן תאריך פתיחה, הסניף עובר לספר שלו מאותו חודש — גם לפני
          שנכנסה בו שקל הכנסה — ומאותו רגע נרשמות אצלו שורות התעריפון וההתחשבנות מול השותף. חודשים שלפני התאריך לא
          מחושבים כלל: אפס הכנסות, אפס הוצאות ואפס העברה לבעלים. סניף בלי תאריך פתיחה נכנס לחישוב רק כשמופיעה בו
          ההכנסה או ההשכרה הראשונה. הסימון <b className="text-ink">&quot;עדיין לא התחיל לפעול&quot;</b> גובר על הכל,
          גם על תאריך פתיחה שכבר הוזן — כך שסניף שנפתח על הנייר בלבד לא מקבל שורות תעריפון (פרסום, סינון וגלישה) ולא
          נוצרת לו דרישת העברה. <b className="text-ink">רק {ownerName} יכול לקבוע ולשנות את השדות האלה</b> — לבעלי
          הסניפים אין גישה אליהם.
        </div>
      </section>
    </div>
  );
