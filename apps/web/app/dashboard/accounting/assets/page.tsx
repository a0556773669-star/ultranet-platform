 ...
 * שכבה 2 במקום אחד.
 *
 * שלושת מסכי הציוד היו שלוש לשוניות נפרדות בשורה העליונה, כלומר שלוש הזדמנויות לתהות באיזו
 * מהן צריך להיות. הם מעשה אחד בשלושה רגעים — ציוד נכנס, זז, ויוצא — ולכן הם יושבים תחת לשונית
 * אחת, בסדר הזה. אף מסך לא נמחק; כולם במרחק לחיצה.
 */
  ...
  const screens = [
    {
      ...
      note: "כאן ציוד נכנס לעסק. חשבונית אחת יוצרת תנועה הונית אחת ופריט לכל יחידה, עם העלות האמיתית שלו.",
      icon: Receipt,
      stat: `${assets.purchases.length} חשבוניות`,
    },
    {
      href: "/dashboard/accounting/inventory",
      label: "מלאי ומשלוחים",
      note: "איפה נמצא כל פריט וכמה הוא עלה. משלוח לסניף לא רושם שקל — העלות פשוט נוסעת עם הפריטים.",
      icon: Boxes,
      stat: `${active} פריטים פעילים`,
    },
    {
      href: "/dashboard/accounting/sales",
      label: "יציאת ציוד",
      note: "מכירה, גריטה ואבדן. תמורה ממכירת ציוד היא החזר הון, לא הכנסה.",
      icon: PackageMinus,
      stat: `${assets.exitedItems.length} יצאו`,
    },
  ];
  ...
  return (
    <div>
      ...
      <div className={`${CARD} mb-3.5 px-4 py-3 text-[12.5px] leading-relaxed text-muted`}>
        ...
        לרווחיות של אף סניף, לא מתחלק עם אף שותף, ומופיע מתחת לשורה התחתונה ולא בתוכה.
      </div>
      ...
      <div className="flex flex-col gap-2.5">
        {screens.map((s) => (
          ...
          >
            ...
            <span className="whitespace-nowrap rounded-full bg-[#f4f6f9] px-2.5 py-1 text-[11px] font-extrabold text-muted">
              {s.stat}
            </span>
          </Link>
        ))}
