...
| `n_quarters` | `Quarter` | רבעוני העבודה (`/dashboard/duxus/rocks`). **מזהה המסמך הוא ה-`quarterKey`** שמופיע על `Rock`/`Milestone`: `label` (תווית חופשית, למשל "ראש חודש אלול - ראש חודש כסלו"), `status` (`active`/`archived` - מאורכב = קריאה בלבד, נאכף בשרת), `startDate`/`endDate` (רשות, לתצוגה), `order` (חותמת זמן למיון ציר הזמן), `rolledFromKey` (הרבעון שממנו גולגל). מפתחות לועזיים ישנים ("2026-Q3") ממשיכים לעבוד בלי מיגרציה - מוצגים כרבעון פעיל וירטואלי, והמסמך נוצר בפועל רק כשמבצעים עליו פעולה |
...
- **`RockMilestoneTree`** (`rock-tree.tsx`) - רכיב עץ משותף לטאבי
  ...
  נפרדות). כל טאב מזין איך לצייר כל אבן דרך (`renderMilestone`), מה להציג
  בשורת הכותרת (`renderRockExtra`) ומה מתחת לאבני הדרך (`renderRockFooter` -
  למשל טופס "+ אבן דרך"). `groupMilestonesByRock` מסננת החוצה משימות שוטפות
  (`splitRockAndAdhoc`/`isAdhoc`) כדי שלא יזהמו את העץ.
  `AddRockForm`/`AddMilestoneForm` (`add-forms.tsx`) משותפים גם הם.
- **`AdhocPanel`** (`adhoc-panel.tsx`, משותף לשלושת הטאבים) - קטע "משימות
  שבועיות ושוטפות" עם כפתור "+ הוסף משימה שבועית/שוטפת" שיוצר `Milestone` עם
  `source: "adhoc"` ובלי `rockId`, בדלי של הטאב הנוכחי (רבעון → `backlog`,
  חודשי → `month`+`monthKey`, שבועי → `week`+`weekKey`). כל משימה כזו מסומנת
  בתג "שוטף" - כך שהמקור (נגזר מסלע / שוטף) ברור בכל מקום.
1. **טאב רבעון** (`/rocks`, `?q=<quarterKey>`) - `quarter-client.tsx`. בלי `?q=`
   נכנסים לרבעון הפעיל האחרון. `QuarterBar` (`quarter-bar.tsx`) בראש המסך: בורר
   רבעונים (כולל מאורכבים, מסומנים "(ארכיון)"), תג פעיל/ארכיון, שינוי שם ותאריכים,
   ארכוב/החזרה לפעיל, וכפתור "פתיחת רבעון חדש". מתחתיו יצירה inline של
   סלע/תת-סלע/אבן-דרך בכל רמה (עדכון אופטימי ב-state המקומי לפני אישור השרת),
   סטטוס סלע לחיצה למחזור `active` → `done` → `dropped`, ומחיקת סלע כוללת מחיקה
   מדורגת (cascade) של תתי-הסלעים ואבני הדרך שבתוכו (`deleteRockAction`). בחירת
   אבני דרך (checkbox, רק כאלה שעדיין ב-`backlog`) + "העברה לחודשי"
   (`promoteMilestonesToMonthAction`).
2. **טאב חודשי** (`/rocks/month`, `?m=YYYY-MM&q=<quarterKey>`) - `month-client.tsx`.
   אותו עץ סלעים בדיוק, עם כל אבני הדרך של הרבעון מוצגות תחתיו: אבני דרך
   ב-`backlog` ניתנות לבחירה + "הוספה לחודש הנוכחי"; אבני דרך ב-`stage: "month"`
   של החודש הנוכחי ניתנות לבחירה + "העברה לשבועי"; השאר read-only עם תג. לכל
   סלע/תת-סלע כפתור "+ אבן דרך לחודש הזה" שיוצר **ישירות** ב-`stage: "month"`.
   קטע "לא הושלמו מחודשים קודמים" עם "העבר לחודש הנוכחי"
   (`carryOverMilestoneToMonthAction`, מגדיל `carryOverCount`).
3. **טאב שבועי** (`/rocks/week`, `?w=Wnnn&q=<quarterKey>` - מפתח שבוע פנימי
   מונוטוני, לא ISO, ראו `date-utils.ts`) - `week-client.tsx`, אותו דגם ברמת
   חודש→שבוע: אבני דרך ב-`stage: "month"` של החודש הנוכחי ניתנות לבחירה +
   "הוספה לשבוע הנוכחי"; אבני דרך ב-`stage: "week"` של השבוע הנוכחי מסומנות
   "בוצע" ישירות (`toggleMilestoneDoneAction`); "+ אבן דרך לשבוע הזה" יוצר
   ישירות ב-`stage: "week"`. קטע "לא הושלמו משבוע שעבר" עם "העבר לשבוע הבא"
   (`carryOverMilestoneToWeekAction`, שבוע אחד קדימה בכל לחיצה - אישור מפורש,
   לא קפיצה אוטומטית להווה).
   **הערה על גזירת הרבעון:** מאחר שלרבעון בעל שם אין קשר לתאריך לועזי, טאבי
   חודשי/שבועי עובדים על **הרבעון הפעיל** כברירת מחדל (ולא על `monthQuarterKey`
   כמו קודם); אפשר לנעול רבעון אחר דרך `?q=`, והוא נשמר בקישורי הקודם/הבא. שם
   הרבעון מוצג מתחת לכותרת התקופה כדי שברור על מה עובדים.
4. **טאב היסטוריה** (`/rocks/history`) - כל אבני הדרך שהושלמו (ירוק, תאריך
   סיום), כל הסלעים מקובצים לפי רבעון (בשמות מ-`n_quarters`, עם תג "ארכיון",
   ממוינים לפי ציר הזמן של הרבעונים), וכל סיכומי הפגישות.
5. **`ReviewPanel`** (`review-panel.tsx`, משותף לשלושת טאבי רבעון/חודש/שבוע)
   - קטע מתקפל "פגישת X" עם מבנה מומלץ לפגישה (רשימת סעיפים סטטית), טקסטאזה
   לסיכום/לקחים, ושמירה ל-`n_rock_reviews` עם מזהה דטרמיניסטי
   `${period}_${periodKey}` (upsert - רשומה אחת בלבד לתקופה). ברבעון מאורכב
   הפאנל עובר ל-`readOnly`.
