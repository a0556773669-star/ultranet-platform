...
שנקבע בכרטיס הסניף (`/dashboard/rentals/branches/[id]` או `/dashboard/branches/[id]`), ומוצג
בכל מקום שבו הסניף מופיע. `computeBranchActivity` (`lib/accounting-overview.ts`) קובע לכל סניף
`startMonth` לפי הסדר: `openedAt` → `founded` (נתונים ישנים) → החודש הראשון שיש בו נתון אמיתי
(השכרה שהוחזרה, הכנסה, הוצאה). לכל חודש נגזר `status`:

| `status` | מתי | מה מחושב |
| --- | --- | --- |
| `active` | החודש ≥ `startMonth` | הכל כרגיל |
| `before_open` | החודש < תאריך הפתיחה | **כלום** — אפס הכנסות, אפס הוצאות, אפס העברה |
| `not_started` | אין תאריך פתיחה **וגם** אין שום נתון בסניף | **כלום** — הסניף מסומן "לא התחיל השכרות" |
