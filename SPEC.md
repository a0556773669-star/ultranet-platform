...
2. **`/branches` — סטטוס סניפים** (owner בלבד): טבלה של כל הסניפים עם תאריך הפתיחה, הסימון
   "עדיין לא התחיל לפעול", מה המערכת מחשבת לכל סניף היום (`startSource`) והמספרים של החודש
   הנבחר — ושמירה פר-שורה. זה המקום שממנו מתקנים תמונה שגויה של סניף שטרם התחיל לעבוד.
...
וסימון "עדיין לא התחיל לפעול" (`notStarted`), שנקבעים במסך **סטטוס סניפים**
...
→ החודש הראשון שיש בו נתון אמיתי (השכרה שהוחזרה, הכנסה, הוצאה); המקור נשמר ב-`startSource`
ומוצג במסך סטטוס הסניפים. לכל חודש נגזר `status`:

| `status` | מתי | מה מחושב |
| --- | --- | --- |
| `active` | החודש ≥ `startMonth` | הכל כרגיל |
| `before_open` | החודש < תאריך הפתיחה | **כלום** — אפס הכנסות, אפס הוצאות, אפס העברה |
| `not_started` | `notStarted` מסומן, **או** שאין תאריך פתיחה וגם אין שום נתון בסניף | **כלום** — הסניף מסומן "לא התחיל השכרות" |
