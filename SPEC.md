...
7. **`/sales` — יציאת ציוד** (owner בלבד): מכירה, גריטה ואבדן — מסך הרכש בכיוון ההפוך.
   יוצר תנועה הונית **נכנסת** אחת, ומסמן כל פריט עם `soldAt`/`soldPrice`/`saleTxId`/`lastBranchId`.
8. **`/review` — סקירת הזנות** (owner בלבד): ההוצאות שמנהלי הסניפים הזינו, עם סימון אוטומטי
   לשורות חריגות. **רשימה, לא שער אישור.**
9. **`/policies` — מי משלם מה** (owner בלבד): ההסכמים של כל הסניפים בטבלה אחת.
10. **`/bottom-line` — השורה התחתונה** (owner בלבד): שורת מאזן אחת לכל אולטרנט, המזכר ההוני
   מתחת לקו, שני הספרים זה לצד זה, וטבלת כל הסניפים ממוינת לפי אחוז החזר השקעה.
11. **`/integrity` — בדיקת שלמות** (owner בלבד): כל הכללים שאפשר לבדוק בקוד, והחריגות מהם.
12. **`/routes` — מסלולי גבייה** (`n_collection_routes`).
13. **`/rates` — תעריפון** (`n_cost_rates`) ו-**`/ads` — פרסום משותף** (`n_ad_areas`).
14. **`/import` — ייבוא הוצאות מאקסל** (owner בלבד): הדבקת טבלה מ-Excel (TSV/CSV, בלי שום תלות
