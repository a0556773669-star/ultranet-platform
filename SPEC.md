...
**מבנה המודול (4 מסכים, ניווט ב-`accounting-tabs.tsx`):**
`/dashboard/accounting` עצמו הוא רק `redirect` ל-`/overview`, כך שלחיצה על "הנה"ח" בתפריט
נוחתת על מסך הסקירה.
1. **`/overview` — סקירה** (המסך הראשי; ראו למטה).
2. **`/entries` — רישום ותנועות** (מה שהיה קודם `/dashboard/accounting`): טופסי הוספת
...
3. **`/routes` — מסלולי גבייה** (`n_collection_routes`).
4. **`/rates` — תעריפון** (`n_cost_rates`) ו-**`/ads` — פרסום משותף** (`n_ad_areas`).
5. **`/import` — ייבוא הוצאות מאקסל** (owner בלבד): הדבקת טבלה מ-Excel (TSV/CSV, בלי שום תלות
