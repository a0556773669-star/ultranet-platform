...
- **`/branches` + `/branches/[id]` + `/branches/new`** (owner בלבד לעריכה) — טופס הסניף
  ...
  במקומו, ולכן שום חישוב קיים לא השתנה.
...
| `apps/web/app/dashboard/(computer-rooms)/branches/setup-cost-field.tsx` | שדה עלות ההקמה בטופס סניף חדר מחשבים: מציג את הסה"כ, וכפתור פותח חלון צד להוספת שורה לכל הוצאה (`Branch.setupItems`). הסה"כ תמיד מחושב מהשורות, לא מוקלד |
