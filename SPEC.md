...
3. **`/entries` — רישום ותנועות** (מה שהיה קודם `/dashboard/accounting`): טופסי הוספת
   הכנסה/הוצאה, גבייה (`collect-modal.tsx`, `lib/collection-charge.ts`) ורשימות ההכנסות/הוצאות
   האחרונות. לכל הוצאה יש כפתור **עריכה** (`edit-expense-modal.tsx`, owner בלבד) לעריכת
   תאריך/תיאור/סכום/עסק (`updateExpenseAction`), בנוסף למחיקה.
4. **`/routes` — מסלולי גבייה** (`n_collection_routes`).
5. **`/rates` — תעריפון** (`n_cost_rates`) ו-**`/ads` — פרסום משותף** (`n_ad_areas`).
6. **`/import` — ייבוא הוצאות מאקסל** (owner בלבד): הדבקת טבלה מ-Excel (TSV/CSV, בלי שום תלות
