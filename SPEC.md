...
טבלה אחת: חודשים לרוחב (חלון של 12, עם ניווט אחורה/קדימה), סניפים לאורך. בכל תא הרווח הנקי
שלי מהסניף באותו חודש מחולק במספר המחשבים שהיו בו באותו חודש — מחשב שנוסף באמצע נספר רק
מהחודש שנוסף בו (`computersActiveInMonth` לפי `addedDate`). ירוק = עומד ביעד
