...
- **`/new`** — יצירת השכרה חדשה (`new-rental-form.tsx`). בחירת הלקוח מתבצעת דרך
  ...
  פריט בו-זמנית. כל פעולת שינוי במודול ההשכרות (`../actions.ts`,
...
- **`/laptops`** — ניהול מחשבים ניידים + תמחור (CRUD). כשמסמנים "יש סטיק משוייך למחשב זה"
  ...
  "סנכרון סטיקים חסרים" בראש `/laptops` (`syncAllSticksAction`) הוא backfill חד-פעמי אידמפוטנטי
  שיוצר סטיק (במחיר 0 עד עריכה) לכל מחשב עם `hasStick` שעדיין אין לו רשומת סטיק מקושרת - נועד
  למחשבים שסומנו כבעלי סטיק לפני שהסינכרון האוטומטי הוגדר.
...
- **`/manage`** — בראש הדף לוח "מצב מחשבים וסטיקים": כרטיסייה לכל מחשב נייד וכרטיסייה
  נפרדת לכל סטיק בסניף, "פנוי" (טורקיז) / "מושכר" + שם השוכר (אדום) לפי השכרה פעילה
  (`n_rentals` עם `kind`/`itemId` תואמים). מחשב שה"פנוי" (אין לו השכרת `laptop` פעילה)
  אך הסטיק המקושר אליו (`n_sticks.linkedLaptopId`) מושכר בנפרד מציג בנוסף שורת אזהרה
  אדומה "ללא אינטרנט" — כלומר המחשב עצמו זמין להשכרה, אבל בלי הסטיק שמספק לו אינטרנט.
