...
**הפקת קבלה על הכנסת ניידים** (`receipt-actions.ts` → `lib/ezcount.ts`): המערכת מקימה את
הלקוח ב-EZcount לפי השם והמייל, מפיקה מסמך מסוג 400 ושולחת אותו במייל באותה קריאה; מספר
המסמך וקישור ה-PDF נשמרים על שורת ההכנסה (`receiptDocNumber` / `receiptPdfUrl`).
...
המשוייך לסניף גובר על מסלול גלובלי. לצד הכפתור יש תיבת סימון ידנית "קבלה", לקבלה שכבר יצאה
במקום אחר — אילוץ להפיק מסמך נוסף רק כדי לסמן וי היה יוצר כפילות בספרים.
